...
int fd;
//array of strings to hold all directory names then print backwards
char *fullpath[10];
int i = 0;
while(i < 10){
	fullpath[i++] = (char*)malloc(sizeof(char)*20);
}
...
while(failsafe < 10){
	...
	if(st.ino == st_parent.ino){
		...
		//concate to the front of the string a ;
		fullpath[failsafe] = "/";
		break; //since we found the root
	}
	else
...
//we have not found hte same inode
	...
	//string concate for path
	
	fullpath[failsafe] = dirname;
	fullpath[failsafe++][strlen(dirname)] = '/';
	printf(1, " cd ..\n\n");
	chdir("..");
}
