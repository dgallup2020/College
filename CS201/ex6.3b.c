...
int main() {
  ...
  */
  ...
  int end = i-1; //index of the last item
  //working?
  int j;
  for(j=0; j<i; j++)
    printf("%s\n", studts[j].fname);
  ...
  for(; end>0;  end--) {
    int mp = posLargest(end);
    //swap
    struct student tmp = studts[mp];
    studts[mp] = studts[end];
    studts[end] = tmp; 
   } 

  for(j=0; j<i; j++)
    printf("%s %s %d\n", studts[j].lname, studts[j].fname, studts[j].standing);
