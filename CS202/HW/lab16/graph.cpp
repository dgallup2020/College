...
//BFS for the Graph Connection check
bool Graph::cBFS(string starts){
  initAlg();  // init all parents to NULL, all dist to 0

  // start vertex
  Vertex *s = lookupV(starts);
  if (s == NULL) return 0;
  s->parent = s; s->dist = 0;
    ...
    for(int i=0; i < E.size(); i++) {
      ...
      if (u != NULL) {
	      if (u->parent == NULL) {
	        ...
	        u->dist = v->dist + 1;
	        frontier.push(u);
	        cout << v->label << " , " << u->label << endl;
	        numbvert++;//cout << numbvert << endl;
	      }
      }
      //if(u. == ) return 0; //what is wrong with this condition. 
    }
    //if(numbvert==0) return 0;
  }
  return 1;
