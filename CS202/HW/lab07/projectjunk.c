...
void reDisplay(struct player p, struct player m1) {
  ...
  mvprintw(27, 0, "HEALTH = %d ",p.health);
...
struct player heal_dc(char in, struct player tmp, struct player m1){
  ...
  if(in == 'w' && (myMaze[tmp.row-1][tmp.col] == m1.sprite[0])) tmp.health-=10; 
  if(in == 's' && (myMaze[tmp.row+1][tmp.col] == m1.sprite[0])) tmp.health-=10;
  if(in == 'a' && (myMaze[tmp.row][tmp.col-1] == m1.sprite[0])) tmp.health-=10;
  if(in == 'd' && (myMaze[tmp.row][tmp.col+1] == m1.sprite[0])) tmp.health-=10;
...
int main(int argc, char *argv[]) {
  ...
  while (chTyped != 'q') {
    ...
    if(p1.health>0){ //character goes through MONSTER because of order in comparision???)//NO
