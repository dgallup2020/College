...
void free_word(word_t *w);           // free memory malloc'ed for struct.  new after the video

void init_word(word_t *w);           // init already malloc'ed

void new_word(word_t *w, char *s);   // put word s into w
void free_string_word(word_t *w);    // free memory malloc'ed for string.  new after the video
